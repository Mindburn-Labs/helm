...
func HELMAPIs() *APIRegistry {
	...
	registry.RegisterAPI(&APIDefinition{
		Name:           "orgvm",
		Description:    "Organizational Virtual Machine API",
	...
	registry.RegisterAPI(&APIDefinition{
		...
		Versions: []APIVersion{
			...
			},
			{
				...
				ReleasedAt: time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
				Changelog:  "Breaking: New SwarmPDP with delegation",
				Breaking:   true,
			},
		},
	...
	registry.RegisterAPI(&APIDefinition{
		Name:           "crypto",
		Description:    "Cryptographic primitives API (PQC, HSM)",
		CurrentVersion: Version{Major: 2, Minor: 0, Patch: 0},
		Stability:      StabilityStable,
		Versions: []APIVersion{
			{
			...
			},
			{
				Version:    Version{Major: 2, Minor: 0, Patch: 0},
				ReleasedAt: time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
				Changelog:  "Go 1.24 native PQC (FIPS 203/204)",
				Breaking:   true,
			},
		},
		DeprecatedAPIs: []DeprecatedAPI{
			{
				...
				Replacement:    "crypto/mlkem.GenerateKey768()",
				Reason:         "Replaced by Go 1.24 native implementation",
				MigrationGuide: "Use the native crypto/mlkem package instead of simulated PQC",
			},
		},
		LastUpdated: time.Now(),
	})
